...
export default function Profile() {
  ...
  const removeFollower = async (person) => {
    try {
      await api.del(`/users/me/followers/${person.id}`);
      setFollowList(prev => prev.filter(p => p.id !== person.id));
      updateUser({ ...user, followers_count: Math.max(0, (user.followers_count || 1) - 1) });
    } catch {}
  };

  const unfollowPerson = async (person) => {
    try {
      await api.post(`/users/${person.username}/follow`, {});
      setFollowList(prev => prev.filter(p => p.id !== person.id));
      updateUser({ ...user, following_count: Math.max(0, (user.following_count || 1) - 1) });
    } catch {}
  };
