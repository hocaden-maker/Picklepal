...
router.post('/', authenticate, (req, res) => {
  ...
  const receiver = db.prepare('SELECT id, display_name FROM users WHERE id = ?').get(receiver_id);
  if (!receiver) return res.status(404).json({ error: 'User not found' });

  const id = uuidv4();
  const text = message?.trim() || `Hey! Want to play pickleball together? 🏓`;
  db.prepare('INSERT INTO invites (id, sender_id, receiver_id, message) VALUES (?, ?, ?, ?)').run(id, req.user.id, receiver_id, text);

  const dmId = uuidv4();
  db.prepare('INSERT INTO dm_messages (id, sender_id, receiver_id, content) VALUES (?, ?, ?, ?)').run(dmId, req.user.id, receiver_id, text);
...
router.put('/:id', authenticate, (req, res) => {
  ...
  db.prepare('UPDATE invites SET status = ? WHERE id = ? AND receiver_id = ?').run(status, req.params.id, req.user.id);
